...
const NeuralMultiplier = () => {
  ...
  const transformContent = useCallback(async (text, platform, tone) => {
    if (!text.trim()) return '';
    
    try {
      const enhancedContent = await generateContent({
        prompt: `Transform this content for ${platform} with a ${tone} tone. Focus on making it engaging and platform-appropriate.\n\nContent: "${text}"`,
        maxTokens: platformPresets[platform]?.maxLength || 500,
        temperature: 0.7,
      });

      // Apply platform-specific formatting
      return platformPresets[platform]?.format(enhancedContent) || enhancedContent;
    } catch (error) {
  ...
  // Debounced content transformation
  const transformContentDebounced = useMemo(
    () =>
      debounce(async (text, platform, tone) => {
        return await transformContent(text, platform, tone);
      }, 500),
    [transformContent]
  );
  ...
  useEffect(() => {
    return () => {
      transformContentDebounced.cancel?.();
    };
  }, [transformContentDebounced]);
  
  // Enhanced content transformation presets
