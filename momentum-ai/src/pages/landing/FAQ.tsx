  ...
  return (
    <section className="relative py-32 section-bg-premium">
      <div className="absolute inset-0 z-0">
        <div className="nebula-glow-enhanced w-[500px] h-[500px] bg-neon-violet top-10 left-10" style={{ opacity: 0.28 }} />
        <div className="nebula-glow-enhanced w-[450px] h-[450px] bg-neon-blue bottom-10 right-10" style={{ opacity: 0.25 }} />
      </div>
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="premium-card premium-card-holographic rounded-xl px-6 group"
              >
